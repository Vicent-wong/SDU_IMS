...
<body>
	...
	<div id="editDialog" style="padding: 20px">
		<div
			style="width: 300px; height: 400px; float: right; margin: 20px 120px 0 0;">
			<img id="user_photo" alt="照片"
				style="margin-bottom: 30px; display: block; max-width: 250px; max-height: 300px;"
				title="照片" src="PhotoServlet?method=GetPhoto" />
			<form id="uploadForm" method="post" enctype="multipart/form-data"
				action="PhotoServlet?method=SetPhoto" target="photo_target">
				<!-- StudentServlet?method=SetPhoto -->
				<input class="easyui-filebox" name="photo"
					data-options="prompt:'选择照片'" style="width: 200px;"> <input
					id="uploadBtn" class="easyui-linkbutton"
		...
		<form id="editForm">
			<table cellpadding="8">
				<tr>
					<td>工号:</td>
					<td><input id="edit_number" value="${userDetail.number }"
				...
				<tr>
					<td>姓名:</td>
					<td><input id="edit_name" value="${userDetail.name }"
				...
				<tr>
					...
					<td><select id="edit_sex" class="easyui-combobox"
						...
						name="sex"><option
								${userDetail.sex == '男'? 'selected':''} value="男">男</option>
							<option ${userDetail.sex == '女'? 'selected':''} value="女">女</option></select></td>
				</tr>
				<tr>
					<td>电话:</td>
					<td><input id="edit_phone" value="${userDetail.phone }"
						style="width: 200px; height: 30px;" class="easyui-textbox"
						type="text" name="phone" validType="mobile" /></td>
				</tr>
				<tr>
					<td>QQ:</td>
					<td><input id="edit_qq" value="${userDetail.qq }"
						style="width: 200px; height: 30px;" class="easyui-textbox"
				...
				<c:forEach var="item" items="${userDetail.courseList}">
					<tr>
						<td>课程:</td>
						<td><input value="${item.grade.name }"
							style="width: 140px; height: 30px;" class="easyui-textbox"
							data-options="readonly: true" />&nbsp;&nbsp; <input
							value="${item.clazz.name }" style="width: 140px; height: 30px;"
							class="easyui-textbox" data-options="readonly: true" />&nbsp;&nbsp;
							<input value="${item.course.name }"
							style="width: 140px; height: 30px;" class="easyui-textbox"
	...
	<div id="passwordDialog" style="padding: 20px">
		<form id="editPassword">
			<table cellpadding="8">
				<tr>
					<td>原密码:</td>
					<td><input id="old_password"
						style="width: 200px; height: 30px;" class="easyui-textbox"
						type="password" validType="oldPassword[${user.password}]"
						data-options="required:true, missingMessage:'请输入原密码'" /></td>
				</tr>
				<tr>
					<td>新密码:</td>
					<td><input type="hidden" name="account"
						value="${userDetail.number }" /> <input id="new_password"
						style="width: 200px; height: 30px;" class="easyui-textbox"
						type="password" validType="password" name="password"
						data-options="required:true, missingMessage:'请输入新密码'" /></td>
				</tr>
				<tr>
					<td>新密码:</td>
					<td><input id="re_password"
						style="width: 200px; height: 30px;" class="easyui-textbox"
